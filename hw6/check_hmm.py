...
class HMMChecker:
    def __init__(self, hmm: str) -> None:
        self.hmm = {}
        self.init_states = {}
        self.trans_states = defaultdict(lambda: defaultdict(float))
        self.emiss_states = defaultdict(lambda: defaultdict(float))
        self.sym_set = set()
        self.state_set = set()
        self.state_num = -1
        self.sym_num = -1
...
{self.warnings["init_warnings"]}{self.warnings["trans_warnings"]}{self.warnings["emiss_warnings"]}
    ...
    def process_transition(self, trans_section: list[str], line_num: int) -> int:
        ...
        while not finished:
            ...
            if line == []:
                ...
                prob = float(line[2])
                self.trans_states[init_state][trans_state] = prob
                i += 1
        for state in self.trans_states:
            trans_prob_sum = 0
            for transition in self.trans_states[state]:
                trans_prob_sum += self.trans_states[state][transition]
    ...
    def process_emission(self, emiss_section: list[str], line_num: int) -> int:
        ...
        while i < len(emiss_section):
            ...
            self.state_set.add(state)
            prob = float(line[2])
            self.emiss_states[state][symbol] = prob
            i += 1
        for state in self.state_set:
            emiss_prob_sum = 0
            for emission in self.emiss_states[state]:
                emiss_prob_sum += self.emiss_states[state][emission]
...
def main():
    with open(args.input, 'r') as hmm:
        checker = HMMChecker(hmm)
